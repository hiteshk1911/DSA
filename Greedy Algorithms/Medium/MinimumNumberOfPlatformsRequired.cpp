...
As soon as the inner loop ends just update the maximum value of count and proceed with the next iteration of the outer loop. After the process ends we will get the maximum value of the count.

Here’s a visualization showing how the two conditions cover all four interval overlap cases:

Let intervals be:


Interval 1: [arr[i], dep[i]]
Interval 2: [arr[j], dep[j]]
Condition 1: arr[i] >= arr[j] && arr[i] <= dep[j]
Condition 2: arr[j] >= arr[i] && arr[j] <= dep[i]
...
Case 1:   |----Interval 1----|
      |--------Interval 2--------|   (Condition 1)

Case 2:     |----Interval 1----|
         |----Interval 2----|         (Condition 1)

Case 3:   |--------Interval 1--------|
                |----Interval 2----|   (Condition 2)
 ...
 int countPlatforms(int n,int arr[],int dep[])
 {
    ...
    for(int i=0;i<=n-1;i++)
    {
        ...
        for(int j=i+1;j<=n-1;j++)
        {
           ...
           (arr[j]>=arr[i] && arr[j]<=dep[i])) //can't we consider 4 cases here -> (arr[j]<=arr[i] && dep[j]>=dep[i]) || (arr[j]<=arr[i] && dep[j]<=dep[i]) || (arr[j]>=arr[i] && dep[j]>=dep[i]) || (arr[j]>=arr[i] && dep[j]<=dep[i]) -> No, because all these 4 cases are covered in the above 2 cases -> how ? -> (arr[j]<=arr[i] && dep[j]>=dep[i]) and (arr[j]>=arr[i] && dep[j]<=dep[i]) are covered in the 2nd case and (arr[j]<=arr[i] && dep[j]<=dep[i]) and (arr[j]>=arr[i] && dep[j]>=dep[i]) are covered in the 1st case
           {
               count++;
           }
        }
