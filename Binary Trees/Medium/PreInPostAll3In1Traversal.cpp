//All 3 In 1 Traversal
Algo:
     Step 1: Initialise a stack that holds a tree node and an integer value representing its state corresponding to pre order, inorder and postorder.
             Initialise empty arrays to store the three traversals as well.Check if the tree is empty. If so, return empty traversals.
     Step 2: Push the root node onto the stack along with its state ‘1’ (preorder) to start the traversal.
     Step 3: While the stack isn’t empty, pop the top node of the stack and for each node:
